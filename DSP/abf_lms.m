% ABF LMS
function [y,e,w]=abf_lms(xn,dn,sps,ref,taps_list,step_len)
%三阶volterra
% 转换为列
xn = xn(:);
dn = dn(:);
...
for idx = 1:n
    ...
    %     fb3_vol = BuildVolterraInput(fb(1:fblen_3),3);
        % 均衡器输出和输入之间的组合
        x_yb= Build_ABF_Input(x1(1:fblen_2),fb(1:fblen_3));
        x=x1.';
        %组合所有输入
        x_all = [x x2_vol x3_vol fb1_vol x_yb];
        w(tapslen_1+1:tapslen_1+fblen_1)=-w(tapslen_1+1:tapslen_1+fblen_1);
        y(idx)=x_all * w;

%     if idx>fblen_1
%         % 均衡器输出和输入之间的组合
%         x_yb= Build_ABF_Input(x1,fb(1:fblen_2));
%         x=x1.';
%         %组合所有输入
%         x_all = [x x2_vol x3_vol -fb1_vol x_yb];
% 
...
%         w = w +  e(idx) * step_len * x_all.';
%     else
%         w(1:tapslen_1)=w(1:tapslen_1)+e(idx) * step_len * x1;
%     end
    %反馈更新
    fb = [fb(2:end) fb_new];
